...
class UserController:
    ...
    def login(self) -> dict:
        try:
            ...
            if not UserUtil.check_password(
                user["password"].encode("utf-8"), self.params.password
            ):
    ...
    def update_plan(self, user_id, plan_id) -> dict:
        try:
            status = self.mysql.update_plan(plan_id, user_id)
    ...
    def get_user_plan_data(self, user_id) -> dict:
        plan = self.mysql.get_user_plan((user_id,))
        return plan
